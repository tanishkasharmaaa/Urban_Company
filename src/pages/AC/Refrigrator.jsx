...
import { refrigratorPackage, selectRefrigrator } from "../../dataBase/refrigrator";
import { CartComponentRefrigrator } from "../../payment/CartComponentRefrigrator";

export function Refrigrator() {
  const [refrigratorCart, setRefrigratorCart] = useState(() => {
    const storedCart = localStorage.getItem('newRefrigratorCart');
    return storedCart ? JSON.parse(storedCart) : [];
  });

  const handleAddToCart = (ele) => {
    setRefrigratorCart((prevCart) => [...prevCart, ele]);
  };

  useEffect(() => {
    localStorage.setItem('newRefrigratorCart', JSON.stringify(refrigratorCart));
  }, [refrigratorCart]);
  ...
  css={{
    ...
    'scrollbar-width': 'none',  
        ...
        <Image
            src="https://res.cloudinary.com/urbanclap/image/upload/t_high_res_category/w_829,dpr_2,fl_progressive:steep,q_auto:low,f_auto,c_limit/images/growth/luminosity/1710395790390-d2a3cc.jpeg"
            borderRadius="md"
            boxShadow="md"
            height='450px'
            w="100%"
