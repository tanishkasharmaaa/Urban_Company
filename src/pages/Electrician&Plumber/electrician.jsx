...
import { electricanPackage, selectElectricianService } from "../../dataBase/electrican";
import { CartComponentElectrician } from "../../payment/CartComponentElectrician";

export function Electrician() {
  const [electricianCart, setElectricianCart] = useState(() => {
    const storedCart = localStorage.getItem('newElectricianCart');
    return storedCart ? JSON.parse(storedCart) : [];
  });

  const handleAddToCart = (ele) => {
    setElectricianCart((prevCart) => [...prevCart, ele]);
  };

  useEffect(() => {
    localStorage.setItem('newElectricianCart', JSON.stringify(electricianCart));
  }, [electricianCart]);
      ...
      >
        <Box flex={{ base: "1", lg: "0 0 30%" }} p={2}>
          ...
          <Box
            ...
            gap={4}
          >
            {selectElectricianService.map((ele, i) => (
              <Box
                key={i}
                alignItems="center"
                textAlign="center"
  ...
  css={{
    ...
    'scrollbar-width': 'none',  
            ...
            <Box flex={{ base: "1", lg: "0 0 30%" }} mt={{ base: 4, lg: 0 }} ml={{ lg: 4 }}>
             <Box bg='lightgray'> <CartComponentElectrician cart={electricianCart} setCart={setElectricianCart} /></Box>
            </Box>
          </SimpleGrid>
        </Box>
      </Box>
