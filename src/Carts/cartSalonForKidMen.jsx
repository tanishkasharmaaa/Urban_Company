...
export function CartSalonForKidMen() {
  const [getSalonForKidMen, setGetSalonForKidMen] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const salon = JSON.parse(localStorage.getItem('newSalonForKidMen')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetSalonForKidMen(salon);
      setAuth(authData);
    };
  ...
  function handleDelete(index) {
    const updatedCart = getSalonForKidMen.filter((_, i) => i !== index);
   setGetSalonForKidMen(updatedCart);
    localStorage.setItem('newSalonForKidMen', JSON.stringify(updatedCart));
  }
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          <Box overflowY="scroll" height={{ base: 'auto', md: '500px' }}>
            {getSalonForKidMen.map((ele, i) => (
              <Box key={i} borderRadius="md" bg="white" boxShadow="md" p={4} mb={4}>
                ...
                <Text mb={2}>{ele.service?.Shave_beard}</Text>
                <Text mb={2}>{ele.service?.haircut}</Text>
                <Text mb={2}>{ele.service?.color}</Text>
                <Text mb={2}>{ele.service?.detan}</Text>
                <AccordionTag />
              </Box>
            ))}
          </Box>
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getSalonForKidMen.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getSalonForKidMen.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
