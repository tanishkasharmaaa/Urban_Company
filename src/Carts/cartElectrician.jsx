...
export function CartElectrician() {
  const [getElectrician, setGetElectrician] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const electrician = JSON.parse(localStorage.getItem('newElectricianCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetElectrician(electrician);
      setAuth(authData);
    };
  ...
  function handleDelete(index) {
    const updatedCart = getElectrician.filter((_, i) => i !== index);
    setGetElectrician(updatedCart);
    localStorage.setItem('newElectricianCart', JSON.stringify(updatedCart));
  }
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getElectrician.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getElectrician.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
