...
export function CartHouseCleaning() {
  const [getHouseCleaning, setGetHouseCleaning] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const sofa = JSON.parse(localStorage.getItem('newHouseCleaningCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetHouseCleaning(sofa);
      setAuth(authData);
    };
  ...
  function handleDelete(index) {
    const updatedCart = getHouseCleaning.filter((_, i) => i !== index);
   setGetHouseCleaning(updatedCart);
    localStorage.setItem('newHouseCleaningCart', JSON.stringify(updatedCart));
  }
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getHouseCleaning.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getHouseCleaning.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
