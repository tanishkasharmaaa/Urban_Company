...
export function CartPlumber() {
  const [getPlumber, setGetPlumber] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const plumber = JSON.parse(localStorage.getItem('newPlumberCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetPlumber(plumber);
      setAuth(authData);
    };
  ...
  function handleDelete(index) {
    const updatedCart = getPlumber.filter((_, i) => i !== index);
    setGetPlumber(updatedCart);
    localStorage.setItem('newPlumberCart', JSON.stringify(updatedCart));
  }
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getPlumber.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getPlumber.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
