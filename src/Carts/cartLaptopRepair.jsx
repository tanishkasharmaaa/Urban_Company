...
export function CartLaptopRepair() {
  const [getLaptopRepair, setGetLaptopRepair] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const laptop = JSON.parse(localStorage.getItem('newLaptopRepairCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetLaptopRepair(laptop);
      setAuth(authData);
    };
  ...
  function handleProceed() {
    if (getLaptopRepair.length === 0) {
      alert('Your cart is empty. Please add services to proceed.');
    setTimeout(()=>{ navigate('/AcRepair')},1000)
    }
  ...
  function handleDelete(index) {
    const updatedCart = getLaptopRepair.filter((_, i) => i !== index);
   setGetLaptopRepair(updatedCart);
    localStorage.setItem('newLaptopRepairCart', JSON.stringify(updatedCart));
  }
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getLaptopRepair.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getLaptopRepair.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
