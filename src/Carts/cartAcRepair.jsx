...
export function CartAcRepair() {
  const [getAcRepair, setGetAcRepair] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const makeup = JSON.parse(localStorage.getItem('newAcRepairCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetAcRepair(makeup);
      setAuth(authData);
    };
    ...
    <>
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getAcRepair.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Input value={`₹${getAcRepair.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}`} isReadOnly />
