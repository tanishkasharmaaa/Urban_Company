...
export function CartWaterPurifier() {
  const [getWaterPurifier, setGetWaterPurifier] = useState([]);
  ...
  useEffect(() => {
    const fetchData = () => {
      const waterPurifier = JSON.parse(localStorage.getItem('newWaterPurifierCart')) || [];
      const authData = JSON.parse(localStorage.getItem('matched'));
      setGetWaterPurifier(waterPurifier);
      setAuth(authData);
    };
  ...
  function handleDelete(index) {
    const updatedCart = getWaterPurifier.filter((_, i) => i !== index);
   setGetWaterPurifier(updatedCart);
    localStorage.setItem('newWaterPurifierCart', JSON.stringify(updatedCart));
  }
        ...
        <Box flex="1" p={4} borderRadius="md" boxShadow="md" bg="gray.50">
          ...
          <Box mt={4} bg="teal" p={{ base: 3, md: 5 }} color="white" borderRadius="md" boxShadow="md" textAlign="center">
            <Text fontSize={{ base: 'md', md: 'lg' }} fontWeight="bold">
              Total: ₹{getWaterPurifier.reduce((acc, curr) => acc + Number(curr.price), 0).toLocaleString()}
            </Text>
          </Box>
        </Box>
      </Box>
